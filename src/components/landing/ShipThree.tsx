 ...
 * Wooden hull + three masts with cream sails and a gold pennant, lit by an
 * upper-right "sun". Slow turntable rotation with a gentle bob and roll.
 * (The original hero ship — reads well in the dark hero panel too.)
 */
export function ShipThree() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // ── Lighting: sky fill + upper-right sun + warm gold rim ──
      ...
      ship.add(new THREE.Mesh(hullGeo, woodMid));
      ...
      flag.position.set(0.1, 0.3 + 2.8 - 0.05, 0);
      ship.add(flag);
      ...
      function animate() {
        ...
        ship.rotation.y = t * 0.32;
        ship.position.y = -0.2 + Math.sin(t * 0.9) * 0.1;
        ship.rotation.z = Math.sin(t * 0.55) * 0.04;
